...
{
	if (image == NULL) {
		hourHand.backgroundColor = [UIColor greenColor].CGColor;
		hourHand.cornerRadius = 2;
	}else{
...
{
	if (image == NULL) {
		minHand.backgroundColor = [UIColor redColor].CGColor;
	}else{
...
- (void) updateClock:(NSTimer *)theTimer{
    ...
    if (!theTimer)  dateComponents = [[NSCalendar currentCalendar] components:(NSHourCalendarUnit | NSMinuteCalendarUnit | NSSecondCalendarUnit) fromDate:self.theDate];
